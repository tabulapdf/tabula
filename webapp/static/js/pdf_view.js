...
Tabula.DocumentView = Backbone.View.extend({ // Singleton
  ...
  initialize: function(stuff){
    ...
    this.listenTo(this.pdf_view, 'toggleRegexOptions', this.render);
  ...
  update_filter_specs: function(data){
    ...
    Object.keys(this.page_views).forEach(function(key){
      ...
      if(page_view.imageIsLoaded && Tabula.pdf_view.components['control_panel'].show_regex_search){
  ...
  render: function(){
    if(!Tabula.LazyLoad){ // old-style, non-lazyload behavior
      ...
      for (var number=Tabula.pdf_view.lazyLoadCursor;number<=_(this.page_views).keys().length;number++){
        var page_view = this.page_views[number];
        if(!page_view) continue; // this is the first render, and there are no pages (probably!)
        var page_el = $('#page-' + number);
        var visible_on_page = page_el.filter(':visible').length;
        if(visible_on_page && Tabula.HideOnLazyLoad){
          ...
          if(Math.abs(Tabula.pdf_view.lazyLoadCursor - number) < Tabula.LazyLoad ) {
            if(page_el.length){
              page_view.$el.show();
              page_view.trigger("toggleRegexOptions");
              // console.log('show ' + number);
            }else{
              this.$el.append(page_view.render().el);
            }
...
//
//   Serves as the Controller in the Model-View-Controller pattern enforced by Backbone. Creates the RegexQueryHandler
...
Tabula.PageView = Backbone.View.extend({ // one per page of the PDF
  ...
  initialize: function(stuff){
    ...
    this.listenTo(Tabula.pdf_view.pdf_document, 'change', this.render );
    this.listenTo(this, 'toggleRegexOptions', this.render )
    ...
    this.listenTo(this.header_view, 'header_resized', _.debounce(this.detect_filter_resize, 100));
    this.listenTo(this.footer_view, 'footer_resized', _.debounce(this.detect_filter_resize, 100));
  },
  ...
  render: function(){
    ...
    if(this.imageIsLoaded && Tabula.pdf_view.components['control_panel'].show_regex_search){
      ...
      this.$el.append(this.footer_view.render().el);
...
Tabula.ControlPanelView = Backbone.View.extend({ // only one
  ...
  toggleRegexOptions: function(){
    if (this.show_regex_search === true){
      Tabula.pdf_view.components['regex_search_view'].$el.hide()
      this.show_regex_search = false;
      this.pdf_view.trigger("toggleRegexOptions");
    }else{
      Tabula.pdf_view.components['regex_search_view'].$el.show()
      this.show_regex_search = true;
      this.pdf_view.trigger("toggleRegexOptions");
    }
  },
  ...
  perform_regex_search: function() {
    ...
    *Determine if regex pattern has already been previously searched for
    ...
    if (this.regex_results_handler.has_same_query(this.regex_query_handler.model.toJSON()) == false) {
      ...
      alert('The requested search by text has already been performed. Please try a different search pattern.')
    }
...
Tabula.RegexCollectionView = Backbone.View.extend({
  ...
  update_regex_search_properties_on_resize: function(filter_data){
    ...
    var new_subsections=new Tabula.Selections();
      ...
      $.ajax({
        ...
        error: function (xhr, status, err) {
          console.log('Error in regex-check-on-resize event: ' + JSON.stringify(err));
  ...
  remove_regex_search: function(event_data,search_to_remove){
    ...
    $.ajax({
      ...
      error: function (xhr, status, err) {
        console.log('Error removing back-end data for: ' +search_to_remove + 'Error message: ' + JSON.stringify(err));
...
Tabula.PDFView = Backbone.View.extend(
  _.extend({
    ...
    render : function(){
      ...
      this.components['regex_search_view'].$el =  this.$el.find("#regex-container");
      this.components['regex_search_view'].render();

      if (!this.components['control_panel'].show_regex_search) this.components['regex_search_view'].$el.hide();
